...
def enrich_sv_dataset(sv: xr.Dataset, echodata, **kwargs) -> xr.Dataset:
    ...
    try:
        ...
        logger.warning(f"Failed to add depth due to error: {str(e)}", exc_info=True)

    try:
        sv = add_location(sv, echodata, **location_args)
    except (KeyError, ValueError) as e:
        logger.warning(f"Failed to add location due to error: {str(e)}", exc_info=True)

    try:
        sv = add_splitbeam_angle(sv, echodata, **splitbeam_args)
    except (KeyError, ValueError) as e:
        logger.warning(f"Failed to add split-beam angle due to error: {str(e)}", exc_info=True)
